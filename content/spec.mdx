...
| **Current Line** | `#6C664B` | 108, 102, 75  | 49°, 18%, 36%  | Semantic current line highlight token |
...
| **Yellow**       | `#846E15` | 132, 110, 21  | 48°, 73%, 30%  | Strings, text content                 |
| **Green**        | `#14710A` | 20, 113, 10   | 114°, 84%, 24% | Functions, methods, inherited classes |
| **Cyan**         | `#036A96` | 3, 106, 150   | 198°, 96%, 30% | Classes, types, support, regex        |
| **Purple**       | `#644AC9` | 100, 74, 201  | 252°, 54%, 54% | Instance reserved words, constants    |
| **Pink**         | `#A3144D` | 163, 20, 77   | 336°, 78%, 36% | Keywords, storage types               |

### Current Line Rendering

`Current Line` and `Comment` intentionally share the same semantic color in both classic variants. In editors that support alpha blending, use the `Current Line` token as a translucent overlay on top of the background.

If the target editor only supports an opaque (solid) line highlight, use these fallback line-highlight colors:

- **Dracula Classic**: `#353747` (same fallback used in [dracula/cursor](https://github.com/dracula/cursor/blob/main/dev/src/classic.yml));
- **Alucard Classic**: `#E2DECA` (same fallback used in [dracula/cursor](https://github.com/dracula/cursor/blob/main/dev/src/alucard.yml)).

These fallback values are implementation guidance for line highlight rendering only. They do not add new syntax tokens to the official palette.
...
| Context                           | Hex       | RGB        | HSL            |
| --------------------------------- | --------- | ---------- | -------------- |
| **Floating interactive elements** | `#343746` | 52, 55, 70 | 230°, 15%, 24% |
| **Background Lighter**            | `#424450` | 66, 68, 80 | 231°, 10%, 29% |
| **Background Light**              | `#343746` | 52, 55, 70 | 230°, 15%, 24% |
...
This specification follows _TextMate_ scoping conventions for consistent highlighting across editors.
...
- Words that reference the current instance: `this`, `self`, `super`;
- Highlight in Purple with Italic styling for consistency across languages.
...
2. **Instance Reserved Words**: Words that interact with the instance (`this`, `self`, `super`) must be highlighted in Purple with Italic styling (see token classification above);
...
- Test against common color vision deficiencies.
...
#### How New Implementations Reach the Website

After an implementation is accepted, the official [Dracula team](https://github.com/dracula/dracula-theme/blob/main/README.md#team) adds it to the [official theme collection](https://github.com/dracula/dracula-theme) and prepares it for publication on the website. The website is updated separately from individual implementation repositories.

Website updates are usually published in batches alongside other implementations and maintenance fixes, so an accepted implementation or merged change may not appear immediately.
...
> See something that could be improved in this documentation? Contribute by [editing this page on GitHub](https://github.com/dracula/draculatheme.com/blob/main/content/spec.mdx).
